...
function chatnext.tpr_send(name, param)
	...
	if minetest.get_player_by_name(receiver) and
	   chatnext.getopt(receiver, 'tpr') ~= 0 then
		minetest.chat_send_player(receiver, sender..
			' is requesting to teleport to you. /tpy to accept')
		minetest.sound_play('chat_next_pm', {to_player = receiver, gain = 0.9})
		minetest.chat_send_player(sender,
			'Teleport request sent! It will time out in '..timeout_delay..' seconds.')
...
function chatnext.tphr_send(name, param)
	...
	if minetest.get_player_by_name(receiver) and
	   chatnext.getopt(receiver, 'tpr') ~= 0 then
		minetest.chat_send_player(receiver, sender..
			' is requesting that you teleport to them. /tpy to accept; /tpn to deny')
		minetest.sound_play('chat_next_pm', {to_player = receiver, gain = 0.9 })
		minetest.chat_send_player(sender,
			'Teleport request sent! It will time out in '..timeout_delay..' seconds.')
...
function chatnext.tpr_accept(name, param)
	...
	if tpr_list[name] == nil and tphr_list[name] == nil then
		minetest.chat_send_player(name, 'Usage: /tpy allows you to accept '..
			'teleport requests sent to you by other players')
		return
	end
